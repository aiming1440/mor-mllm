...
def draw(args):
    ...
    for k, v in tqdm(data.items()):
        ...
        if images is not None:
            ...
            text_token_path = torch.stack(text_token_path).T  # each token's path across layers
            img_token_path = torch.stack(img_token_path).T  # each token's path across layers
