...
if a == '4' and int(b) >= 36:
    ...
    from mor_mllm.model.language_model.llava_phi_mor import EvalMoRLLaVAPhiForCausalLM
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            # =============================================================================================
                ...
                if getattr(cfg_pretrained, 'mor', {}).get('mor_enable', False):
                    model = EvalMoRLLaVAPhiForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
                    import deepspeed
            ...
            if 'mpt' in model_name.lower():
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, padding_side=padding_side)
                # print(tokenizer)
                # For a full (non-LoRA) checkpoint, use the `mor.mor_enable` flag from
                # the config (rather than checking the model_name string for "mor") to
                # decide whether to reconstruct the MoR structure via
                # EvalMoRLLaVAPhiForCausalLM. Otherwise the checkpoint would be loaded
                # as a plain LlavaPhiForCausalLM (losing the MoR recursion structure,
                # and the weight keys would also fail to match).
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                if getattr(cfg_pretrained, 'mor', {}).get('mor_enable', False):
                    model = EvalMoRLLaVAPhiForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
                else:
